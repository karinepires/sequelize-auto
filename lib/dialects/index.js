...
exports.mysql = {
  ...
  getForeignKeysQuery: function(tableName, schemaName) {
    return "SELECT \
      ...
      , C.extra \
      , M.CONSTRAINT_TYPE as constraint_type \
      FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS K \
      LEFT JOIN INFORMATION_SCHEMA.COLUMNS AS C \
        ON C.TABLE_NAME = K.TABLE_NAME AND C.COLUMN_NAME = K.COLUMN_NAME \
      LEFT JOIN INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS M \
        ON K.TABLE_NAME = M.TABLE_NAME AND K.CONSTRAINT_NAME = M.CONSTRAINT_NAME \
      WHERE \
  ...
  /**
   * Determines if record entry from the getForeignKeysQuery
   * results is an actual unique key
   *
   * @param {Object} record The row entry from getForeignKeysQuery
   * @return {Bool}
   */
  isUniqueKey: function(record) {
